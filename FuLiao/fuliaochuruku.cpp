    ...
    ui(new Ui::FuLiaoChuRuKu)
{
    ...
    QStringList head ;
    head << SS("入库单号") << SS("辅料名称") << SS("剩余数量")<<SS("单位") <<SS("明细");
...
void FuLiaoChuRuKu::FuLiaoChukuSlot(){
    ...
    if(s1 && s2 ){
        QMessageBox::about(NULL, SS("提示"), SS("辅料出库成功"));
        m_MysqlOperate->Commit();
    }else{
        QMessageBox::about(NULL, SS("提示"), SS("辅料出库失败"));
...
bool FuLiaoChuRuKu::ChukuSlotValider(QMap<QString,QString> &RuKudata){
    ...
    if(RuKudata.size() == 0 ){
        QMessageBox::about(NULL, SS("错误提示"), SS("查询不到辅料入库单号"));
        return false;
    }
